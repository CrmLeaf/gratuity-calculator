...
namespace Crmleaf\Payroll\Tools\GratuityCalculator\Tests;

use Crmleaf\Payroll\Calculators\GratuityCalculator;
use Crmleaf\Payroll\Tools\GratuityCalculator\GratuityCalculatorServiceProvider;
 ...
 */
final class GratuityCalculatorTest extends TestCase
{
    /**
     * Held as a literal rather than as GratuityCalculator::class so that a calculator
     * which does not exist fails as one readable assertion instead of a fatal
     * error that takes the rest of the file with it.
     */
    private const CALCULATOR = 'Crmleaf\Payroll\Calculators\GratuityCalculator';
    ...
    {
        return [GratuityCalculatorServiceProvider::class];
    }
    ...
    protected function defineEnvironment($app): void
    {
        ...
        // pins down.
        $app['config']->set('gratuity-calculator.route.enabled', true);
    ...
    {
        $calculator = $this->app->make(GratuityCalculator::class);

        self::assertInstanceOf(GratuityCalculator::class, $calculator);
        self::assertSame($calculator, $this->app->make(GratuityCalculator::class), 'The binding should be a singleton.');
    }

    public function test_the_configuration_is_merged(): void
    {
        self::assertSame('gratuity-calculator', $this->app['config']->get('gratuity-calculator.route.name'));
        self::assertSame('Gratuity Calculator', $this->app['config']->get('gratuity-calculator.view.title'));
    }

    public function test_the_route_is_off_until_it_is_asked_for(): void
    {
        /** @var array{route: array{enabled: bool}} $shipped */
        $shipped = require __DIR__.'/../config/gratuity-calculator.php';
    ...
    {
        $html = Blade::render('<x-crmleaf::gratuity-calculator />');

        self::assertStringContainsString('data-crmleaf-tool="gratuity-calculator"', $html);
    ...
    {
        $response = $this->postJson('/tools/gratuity-calculator', [
        ...
        $response->assertJsonPath('tool', 'gratuity-calculator');
    }

    public function test_incomplete_input_is_rejected_rather_than_guessed_at(): void
    {
        $this->postJson('/tools/gratuity-calculator', [])
            ->assertStatus(422)
            ->assertJsonValidationErrors(['last_drawn_salary']);
    }
}
