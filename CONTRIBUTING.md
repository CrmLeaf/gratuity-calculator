# Contributing to Gratuity Calculator
...
```bash
git clone https://github.com/crmleaf/gratuity-calculator.git
...
requires. `Crmleaf\Payroll\Calculators\GratuityCalculator` is the class doing the work; everything else
...
Made in the monorepo, against `tools/gratuity-calculator/`:
...
```
fix(gratuity): <what changed>
test(gratuity): <the case now covered>
```
