...
namespace Crmleaf\Payroll\Tools\GratuityCalculator\Http\Controllers;
...
use Crmleaf\Payroll\Tools\GratuityCalculator\Http\Requests\GratuityCalculatorRequest;
use Illuminate\Contracts\View\View;
use Illuminate\Http\JsonResponse;

/**
 * The whole HTTP surface of Gratuity Calculator: one action, one calculator call.
 *
 ...
 * payroll - that all lives in Crmleaf\Payroll\Calculators\GratuityCalculator - which is why the tool can
 * be embedded, mounted at any prefix, or ignored entirely in favour of calling
 * the calculator yourself.
 */
final class GratuityCalculatorController
{
    public function __construct(
        private readonly GratuityCalculator $calculator,
    ) {
    }

    public function __invoke(GratuityCalculatorRequest $request): JsonResponse|View
    {
    ...
    private function view(GratuityCalculatorRequest $request, ?CalculationResult $result, ?string $error = null): View
    {
        /** @var \Illuminate\Contracts\Config\Repository $config */
        $config = app('config');

        return view('gratuity-calculator::gratuity-calculator', [
            'result' => $result,
            'error' => $error,
            'input' => $request->submitted() ? $request->validated() : [],
            'defaults' => (array) $config->get('gratuity-calculator.defaults', []),
            'title' => (string) $config->get('gratuity-calculator.view.title', 'Gratuity Calculator'),
            'action' => $request->url(),
        ]);
    }
}
