...
<head>
    ...
    <title>{{ $title ?? 'Gratuity Calculator' }}</title>
    <meta name="description" content="Payment of Gratuity Act 1972 formula with the exemption cap applied.">
    <style>
...
<footer class="crmleaf-tool__colophon">
    ...
    <p>
        Computed by <a href="https://github.com/crmleaf/gratuity-calculator">crmleaf/gratuity-calculator</a>,
...
{{--
    ...
    The bundled script binds any element carrying data-crmleaf-tool="gratuity-calculator",
    computes on submit, and only falls back to posting the form if the browser
    build is not present.
--}}
@if (config('gratuity-calculator.assets.script', true))
    @if (config('gratuity-calculator.assets.cdn'))
        <script src="{{ config('gratuity-calculator.assets.cdn') }}" defer></script>
    @endif
    <script src="{{ asset('vendor/gratuity-calculator/gratuity-calculator.js') }}" defer></script>
@endif
